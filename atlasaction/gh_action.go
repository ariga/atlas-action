...
// IsCoAuthored determines if the given commit on the given PR is a result of a code suggestion made by Atlas.
// There is no to unequivocally determine this, because GitHub is not providing this information, but we are using a set of clues and take an education guess.
func (c *ghClient) IsCoAuthored(ctx context.Context, tc *TriggerContext) (bool, error) {
	if tc == nil || tc.PullRequest == nil {
		return false, nil
	}
	comments, err := c.ReviewComments(ctx, tc.PullRequest.Number)
	if err != nil {
		return false, err
	}
	// Filter out comments that do no contain code suggestions.
	comments = slices.DeleteFunc(comments, func(comment github.PullRequestComment) bool {
		startIdx := strings.Index(comment.Body, "```suggestion")
		if startIdx == -1 {
			return true
		}
		endIdx := strings.LastIndex(comment.Body, "```")
		if endIdx == -1 {
			return true
		}
		if endIdx < startIdx {
		...
		return !strings.Contains(comment.Body, "<!-- generated by ariga/atlas-action for")
	})
	if len(comments) == 0 {
		return false, nil
	}
	...
	if err := c.GetURL(ctx, fmt.Sprintf("repos/%s/commits/%s", tc.Repo, tc.Commit), &resp); err != nil || len(resp.Files) == 0 {
		return false, err
	}
	// All files in the commit must be modified.
	if slices.ContainsFunc(resp.Files, func(f github.PullRequestFile) bool {
		return f.Status != "modified"
	}) {
		return false, nil
	}
	// For each file in the commit, there must be a matching comment.
	for _, file := range resp.Files {
		lines := make([]string, 0)
		for _, l := range strings.Split(file.Patch, "\n") {
			if strings.HasPrefix(l, "+") {
				lines = append(lines, l[1:])
			}
		}
		if !slices.ContainsFunc(comments, func(comment github.PullRequestComment) bool {
			suggestion := strings.Split(strings.Split(comment.Body, "```suggestion")[1], "```")[0]
			return strings.Contains(suggestion, strings.Join(lines, "\n"))
		}) {
			return false, nil
		}
	}
