...
func (c *ghClient) CommentSchemaLint(ctx context.Context, tc *TriggerContext, r *SchemaLintReport) error {
	...
	if err = c.upsertComment(ctx, tc.PullRequest, id, comment); err != nil {
		return fmt.Errorf("failed to upsert comment: %w", err)
	}
	files, err := c.ListPullRequestFiles(ctx, tc.PullRequest.Number)
	if err != nil {
		return fmt.Errorf("failed to list pull request files: %w", err)
	}
	tc.Act.Debugf("PR files: %v", files)
	reviewComments, err := c.ReviewComments(ctx, tc.PullRequest.Number)
	if err != nil {
		return fmt.Errorf("failed to list review comments: %w", err)
	}
	tc.Act.Debugf("Fetched %d review comments", len(reviewComments))
	var errs []string
	...
	for _, step := range r.Steps {
		for _, diag := range step.Diagnostics {
			...
			if diag.Pos.Filename = path.Join(tc.Act.GetInput("working-directory"), diag.Pos.Filename); slices.Contains(files, diag.Pos.Filename) {
				...
				tc.Act.Debugf("Diagnostic with marker: %s for file: %s", marker, diag.Pos.Filename)
			}
		}
	}
	...
	for _, cmt := range reviewComments {
		...
		if len(lines) > 0 {
			...
			if strings.HasPrefix(lastLine, "<!-- generated by ariga/atlas-action for schema-lint") {
				if _, ok := currentMarkers[lastLine]; !ok {
					tc.Act.Debugf("Deleting outdated review comment ID: %d", cmt.ID)
					if err := c.DeleteReviewComment(ctx, cmt.ID); err != nil {
						errs = append(errs, fmt.Sprintf("failed to delete outdated review comment: %v", err))
					}
				}
			}
		}
	...
	tc.Act.Debugf("CommentSchemaLint completed successfully")
	return nil
}
