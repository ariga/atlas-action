...
func (a *Actions) schemaApplyWithApproval(ctx context.Context) ([]*atlasexec.SchemaApply, error) {
	...
	var repo string
	u, err := url.Parse(a.GetInput("to"))
	if err != nil {
		return nil, fmt.Errorf("failed to parse the URL input: %w", err)
	}
	if u.Scheme == "atlas" {
		...
		if err := a.waitingForApproval(func() (bool, error) {
			...
			if err == ErrApprovalTimeout {
				return nil, fmt.Errorf(
					`the schema plan %q was not approved within the specified waiting period. Please manually approve the plan and re-run the action.
You can approve the plan by visiting: %s`, f.Name, f.Link)
	...
	createApprovalPlan := func() ([]*atlasexec.SchemaApply, error) {
		...
		name := fmt.Sprintf("commit-%s-%08x", tc.Commit, hash)
		if tc.PullRequest != nil {
	...
	// Let's Atlas decide what to do with the existing plans.
	case len(plans) > 1:
		return a.Atlas.SchemaApplySlice(ctx, a.schemaApplyParams())
