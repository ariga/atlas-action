...
func (a *Actions) Copilot(ctx context.Context) error {
	...
	case a.GetBoolInput("gen-test") && tc.PullRequest != nil:
		...
		if _, err = a.exec(ctx, "git", "checkout", "-b", b); err != nil {
		...
		if _, err = a.exec(ctx, "git", "commit", "--message", msg); err != nil {
			return fmt.Errorf("failed to commit changes: %w", err)
		}
		if _, err = a.exec(ctx, "git", "push"); err != nil {
...
If you have any questions or want me to edit the test, you can comment on this PR and mention me with /atlas.`, tc.PullRequest.URL)
		pr, err := c.CreatePullRequest(ctx, b, tc.DefaultBranch, msg, body)
		if err != nil {
		...
		return c.CommentCopilot(ctx, tc.PullRequest.Number, &Copilot{Copilot: atlasexec.Copilot{{
			Content: "I have added the generated test cases to a new branch and created a pull request for you. You can review the changes and merge them if you are satisfied.",
		}}})
	// Atlas Copilot will react to comments that mention it by /atlas.
	case tc.Comment == nil || !strings.Contains(tc.Comment.Body, "/atlas"):
		params.Prompt = tc.Comment.Body
		cp, err := a.Atlas.Copilot(ctx, params)
		if err != nil {
			a.SetOutput("error", err.Error())
			return err
