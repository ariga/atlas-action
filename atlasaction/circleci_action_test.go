...
func Test_circleCIOrb_GetTriggerContext(t *testing.T) {
	...
	_, err = orb.GetTriggerContext()
	require.EqualError(t, err, "missing CIRCLE_REPOSITORY_URL environment variable")
	t.Setenv("CIRCLE_REPOSITORY_URL", "https://github.com/ariga/atlas-orb")
	_, err = orb.GetTriggerContext()
	...
	_, err = orb.GetTriggerContext()
	require.EqualError(t, err, "failed to get open pull requests: missing GITHUB_TOKEN environment variable")
	t.Setenv("GITHUB_TOKEN", "1234567890")
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		require.Equal(t, "/repos/ariga/atlas-orb/pulls", r.URL.Path)
		require.Equal(t, "GET", r.Method)
		require.Equal(t, "state=open&head=main", r.URL.RawQuery)
		w.Write([]byte(`
		[
