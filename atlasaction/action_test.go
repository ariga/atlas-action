...
feunc TestMigrateLint(t *testing.T) {
	...
	token := "123456789"
	// srvDir returns a new server that serves graphql queries that list content of the directory at dirPath
	srvDir := func(dirPath string) *httptest.Server {
	...
	srv := srvDir("testdata/migrations")
	...
	t.Run("set working directory input", func(t *testing.T) {
		tt := newT(t)
		srv = srvDir("migrations")
		tt.setupConfigWithLogin(t, srv.URL, token)
		tt.setInput("dev-url", "sqlite://file?mode=memory")
