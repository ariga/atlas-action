...
func TestMigrateLint(t *testing.T) {
	...
	t.Run("lint summary - lint error", func(t *testing.T) {
		...
		require.NoError(t, err)
		sum := string(c)
		require.Contains(t, sum, "`atlas migrate lint` on `testdata/migrations_destructive` results:\n")
	...
	t.Run("lint summary - with diagnostics", func(t *testing.T) {
		...
		require.NoError(t, err)
		sum := string(c)
		require.Contains(t, sum, "`atlas migrate lint` on `testdata/diagnostics` results:\n")
		require.Contains(t, sum, "2 new migration files detected")
	...
	t.Run("lint summary - lint success", func(t *testing.T) {
		...
		require.NoError(t, err)
		sum := string(c)
		require.Contains(t, sum, "`atlas migrate lint` on `testdata/migrations` results:\n")
	...
	t.Run("lint comment", func(t *testing.T) {
		...
		require.Contains(t, createdComment.Body, "No issues found.")
		require.Contains(t, createdComment.Body, "generated by ariga/atlas-action for test-dir-slug")
		require.Equal(t, http.MethodPost, createdComment.Method)
		// Run Lint but this time with lint errors expected
		tt.setInput("dir", "file://testdata/migrations_destructive")
		err = MigrateLint(context.Background(), tt.cli, tt.act)
		require.ErrorContains(t, err, "https://migration-lint-report-url")
		require.Equal(t, 4, len(ghPayloads))
		createdComment = ghPayloads[len(ghPayloads)-1]
		require.Contains(t, createdComment.Body, "1 error found")
		require.Contains(t, createdComment.Body, "generated by ariga/atlas-action for test-dir-slug")
		require.Equal(t, http.MethodPost, createdComment.Method)
		...
		createdComment = ghPayloads[len(ghPayloads)-1]
		require.Contains(t, createdComment.Body, "1 error found")
		require.Contains(t, createdComment.Body, "generated by ariga/atlas-action for other-dir-slug")
		require.Equal(t, http.MethodPatch, createdComment.Method)
		require.ErrorContains(t, err, "https://migration-lint-report-url")
