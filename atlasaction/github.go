...
func (c *GitHubClient) CommentCopilot(ctx context.Context, tc *TriggerContext, cp *Copilot) error {
	return c.CreateIssueComment(ctx, tc.Comment.Number, fmt.Sprintf(
		"> %s\n\n%s\n\n<!-- generated by ariga/atlas-action/copilot for session %s -->",
		cp.Prompt, cp.Copilot.String(), func() string {
			if len(cp.Copilot) > 0 {
				return cp.Copilot[0].SessionID
			}
			return ""
		}(),
	))
