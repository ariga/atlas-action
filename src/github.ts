...
import {CloudReportsPayload} from "./cloud";
...
export function summarize(s: Summary, c?: CloudReportsPayload, cloudURL?: string): void {
  ...
  if (c) {
    for (const cloudReport of c.runs.edges.node.cloudReports) {
      const diags: string[] = []
      ...
      rows.push([icon('warning'), "cloud anlayzers", cloudReport.text, diags.join('\n\n')])
    }
  }
